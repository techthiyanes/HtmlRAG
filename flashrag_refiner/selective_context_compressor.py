...
import time
import torch
from transformers import GPT2Tokenizer, GPT2LMHeadModel, BertTokenizer, AutoTokenizer
...
class SelectiveContext:
    ...
    def __init__(self, model_type="gpt2", model_path="openai-community/gpt2", lang="en", chat_tokenizer_name="llama"):
        ...
        if chat_tokenizer_name == "llama":
            tokenizer_path = "../../huggingface/Meta-Llama-3.1-70B-Instruct/"
            self.chat_tokenizer = AutoTokenizer.from_pretrained(tokenizer_path)
        else:
    ...
    def __call__(self, text: str, reduce_ratio: float = 0.5, target_token: int = 2000, reduce_level: str = "phrase") -> List[str]:
        context = self.beautify_context(text)

        # self.mask_ratio = reduce_ratio
        context_token_len = len(self.chat_tokenizer.encode(context))
        self.mask_ratio = target_token / context_token_len
