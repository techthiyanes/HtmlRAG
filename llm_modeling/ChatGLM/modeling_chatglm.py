...
from torch.nn import CrossEntropyLoss, LayerNorm, MSELoss, BCEWithLogitsLoss, Module
...
from .seq_para_utils import get_sequence_parallel_group, get_sequence_parallel_size, get_sequence_parallel_rank, \
    initialize_seq_parallel
...
initialize_seq_parallel(int(os.getenv("SEQ_PARALLEL_SIZE", 1)))


# assert get_sequence_parallel_group() is not None, f"{get_sequence_parallel_group()}"
...
class DistributedAttention(torch.nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, *args: Any, **kwargs) -> Tensor:
        ...
        origin_batch_size, origin_num_head, origin_seq_len = query.size(0), query.size(1), query.size(2)
        ...
        query_layer = _SeqAllToAll.apply(self.spg, query, self.scatter_idx, self.gather_idx).transpose(0, 1).transpose(
            1, 2).contiguous()
        key_layer = _SeqAllToAll.apply(self.spg, key, self.scatter_idx, self.gather_idx).transpose(0, 1).transpose(
            1, 2).contiguous()
        value_layer = _SeqAllToAll.apply(self.spg, value, self.scatter_idx, self.gather_idx).transpose(0, 1).transpose(
            1, 2).contiguous()
        context_layer = self.local_attn(query_layer, key_layer, value_layer, *args, **kwargs)
        context_layer = context_layer.transpose(0, 1).contiguous()
        # [seq_len, batch_size, num_head, head_dim]
        output = _SeqAllToAll.apply(self.spg, context_layer, self.gather_idx, self.scatter_idx)
        return output.transpose(0, 1)[:, :, :origin_num_head, :].reshape(origin_batch_size, origin_seq_len, -1)
...
class CoreAttention(torch.nn.Module):
    def __init__(self, config: ChatGLMConfig, layer_number):
        ...
        #     projection_size = projection_size // sp_size

        # Per attention head and per partition values.
        self.hidden_size_per_partition = projection_size  # np
        self.hidden_size_per_attention_head = config.kv_channels  # hn
...
class FlashAttention2(CoreAttention):
    ...
    def forward(self, query_states, key_states, value_states, attention_mask):
        ...
        if not self._flash_attn_uses_top_left_mask:
            ...
            causal = self.is_causal and seq_len != 1
        ...
        if get_sequence_parallel_group() is not None:
            ...
            attn_output = attn_output.reshape(batch_size, seq_len, num_heads, head_dim).contiguous()
        else:
            attn_output = attn_output.reshape(batch_size, seq_len, self.hidden_size_per_partition).contiguous()
        return attn_output
...
class SelfAttention(torch.nn.Module):
    ...
    def __init__(self, config: ChatGLMConfig, layer_number, device=None):
        ...
        #  add sequence parallel group
        if get_sequence_parallel_group() is not None:
            # print(f"Using sequence parallel group for attention layer {layer_number}")
            self.core_attention = DistributedAttention(
                CORE_ATTENTION_CLASSES[config._attn_implementation](config, self.layer_number),
                get_sequence_parallel_group(),
            )
        else:
            logger.info(f"_SEQUENCE_PARALLEL_GROUP: {get_sequence_parallel_group()}")
...
class GLMTransformer(torch.nn.Module):
    ...
    def __init__(self, config: ChatGLMConfig, device=None):
        ...
        # print(f"core attention implementation: {config._attn_implementation}")
...
class ChatGLMModel(ChatGLMPreTrainedModel):
    def __init__(self, config: ChatGLMConfig, device=None, empty_init=True):
        ...
        loguru.logger.info(f"Using core attention implementation: {config._attn_implementation}")
    ...
    def forward(
        ...
        if inputs_embeds is None:
            if get_sequence_parallel_group() is not None:
                ...
                inputs_embeds = self.embedding(
                    input_ids[:, seq_length // sp_size * seq_rank:seq_length // sp_size * (1 + seq_rank)])
            else:
        ...
        #  apply sequence parallel group for rotary positional embeddings
        if get_sequence_parallel_group() is not None:
            seq_rank = get_sequence_parallel_rank()
            sp_size = get_sequence_parallel_size()
            kv_seq_len = seq_length // sp_size
            rotary_pos_emb = rotary_pos_emb[:, kv_seq_len * seq_rank: kv_seq_len * (1 + seq_rank), ...]
...
class ChatGLMForConditionalGeneration(ChatGLMPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if get_sequence_parallel_group() is not None:
                part_seq_len = hidden_states.size(1)
                total_seq_len = labels.size(-1)
                seq_rank = get_sequence_parallel_rank()
                start = seq_rank * part_seq_len
                end = min(seq_rank * part_seq_len + part_seq_len, total_seq_len - 1)
                shift_logits = lm_logits[..., :end - start, :].contiguous()
                shift_labels = labels[..., start + 1:end + 1].contiguous()
            else:
    ...
    @staticmethod
    def _reorder_cache(past: Tuple[Tuple[torch.Tensor, torch.Tensor], ...], beam_idx: torch.LongTensor
...
class ChatGLMForHTMLTreeGeneration(ChatGLMForConditionalGeneration):
    def __init__(self, config: ChatGLMConfig, empty_init=True, device=None):
        super().__init__(config)
